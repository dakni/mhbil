...
# this image is created by the chap04 script
load("ws/ws04.rws")
...
# Regression T1, T2
pdf("pictures/c5_T1T2vil.pdf", height=4, width=6, bg = "white") 
...
pdf("pictures/c5_T3vil.pdf", height=4, width=6, bg = "white") 
    plot(X- 1259,E_vil,col="black", pch=16)
    abline(T3_vil)
dev.off() 
...
pdf("pictures/c5_T5vil.pdf", height=4, width=6, bg = "white") 
    plot(X- 1259,E_vil,col="black", pch=16)
    lines(yr,fT5_vil)
dev.off() 
...
chi
pdf("pictures/c5_chi.pdf", height=4, width=6, bg = "white") 
...
r
pdf("pictures/c5_r.pdf", height=4, width=6, bg = "white") 
...
pdf("pictures/c5_boot.pdf", height=4, width=6, bg = "white") 
...
pdf("pictures/c5_decomp.pdf", height=4, width=6, bg = "white") 
...
meg_kde_samp <- sp::over(meg_kde_samppoints, df_dens_r)
vt2    <- variogram(meg_kde_samp$v ~ 1, meg_kde_samppoints)
v.fit2 <- fit.variogram(vt2, vgm(1, "Gau", 5000, 1), fit.sills = TRUE, fit.ranges = TRUE, fit.method = 7)
plot(vt2,v.fit2)
k2     <- krige(meg_kde_samp$v ~ 1, meg_kde_samppoints, df_dens_r, v.fit2, nmin = 3, maxdist = 10000, nmax = 8)
image(k2, col = gray.colors(25, start = 0.97,  end = 0.4))
contour(k2, add=T)    
points(ppp_meg$x, ppp_meg$y, pch=16, cex=0.4) 

save.image("ws/ws05.rws")
